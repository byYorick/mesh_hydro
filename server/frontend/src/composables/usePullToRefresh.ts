 ...
 */
export function usePullToRefresh(onRefresh: any, options: any = {}): any {
  const {
    threshold = 80,
    resistance = 2.5,
    maxDistance = 120,
  ...
  const isPulling = ref<any>(false)
  const pullDistance = ref<any>(0)
  const isRefreshing = ref<any>(false)

  let startY = 0
  let currentY = 0
  let scrollable = null

  function onTouchStart(e: any): any {
  ...
  function onTouchMove(e: any): any {
