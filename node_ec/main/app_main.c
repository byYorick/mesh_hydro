...
void app_main(void)
{
    ...
    ret = node_config_load(&s_node_config, sizeof(ec_node_config_t), "ec_ns");
    if (ret != ESP_OK) {
    ...
    // [Step 5/8] Pumps init (3 насоса)
    ESP_LOGI(TAG, "[Step 5/8] Pumps init (3x PWM)...");
    ESP_ERROR_CHECK(pump_controller_init());
    ESP_LOGI(TAG, "  - 2 pumps ready (GPIO 2,3,4)");
    ...
    // [Step 7/8] EC Manager init
    ESP_LOGI(TAG, "[Step 7/8] EC Manager init...");
