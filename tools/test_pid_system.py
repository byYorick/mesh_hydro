#!/usr/bin/env python3
"""
Тестирование PID системы с моковыми данными pH
"""
...
def send_telemetry(ph_value, node_id=NODE_ID):
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/telemetry", json=data, timeout=5)
        if response.status_code == 200:
            print(f"✅ Телеметрия отправлена: pH={ph_value}")
        else:
            print(f"❌ Ошибка отправки телеметрии: {response.status_code}")
...
def send_event(level, message, ph_value, node_id=NODE_ID):
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/events", json=data, timeout=5)
        if response.status_code == 200:
...
def get_node_status():
    ...
    try:
        response = requests.get(f"{API_BASE}/nodes/{NODE_ID}", timeout=5)
        if response.status_code == 200:
