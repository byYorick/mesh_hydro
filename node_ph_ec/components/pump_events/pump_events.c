/**
 * @file pump_events.c
 * @brief Реализация системы событий для насосов с данными PID
 */
...
static const char *TAG = "pump_events";
...
/**
 * @brief Создание JSON сообщения события
 */
...
static esp_err_t send_event_to_server(const pump_event_t *event) {
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "📤 Pump event sent: %s (pump %d)", 
                 event->type == PUMP_EVENT_START ? "START" : "STOP", event->pump_id);
    } else {
        ESP_LOGW(TAG, "Failed to send pump event: %s", esp_err_to_name(err));
    }
...
esp_err_t pump_events_init(void) {
    ESP_LOGI(TAG, "Pump events system initialized");
    return ESP_OK;
}
